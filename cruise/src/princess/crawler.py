...
class Crawler:
    def __init__(self):
        self.host = "http://www.princess.com"

    def run(self, page):
        session = requests.session()
        session.get(self.host + '/find/searchResults.do')

        size = 100
        url = self.host + '/find/pagination.do?searchCriteria.currency=USD&searchCriteria.startIndex=' + str(
                (page - 1) * size) + '&searchCriteria.endIndex=' + str(
                page * size)
